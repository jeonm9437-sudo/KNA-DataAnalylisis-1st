...
# 가져온 정보(파인 접근 열쇠 / 참조값) f 에 담는다
f = open("data/sample.txt", "r", encoding="utf-8")

print(type(f).__name__)  # 타입의 이름

# 텍스트 파일 한줄씩 문자열을 만들기
lines = f.readlines()
print(lines)

f.close()  # 열었다면 언젠가는 꼭 닫기

# 만약 신경써서 파일닫기(close) 해주기 귀찮다면
# with open ... as 문법을 쓰는 것도 좋음

with open("data/sample.txt", "r", encoding="utf-8") as f:
    # 앞으로 이렇게 들여쓰기 된 코드가 끝나면
    # 파일 접근을 닫습니다(close)

    # 텍스트 파일 한줄씩 문자열을 만들어 리스트 만들기
    lines = f.readlines()
    print(lines)
